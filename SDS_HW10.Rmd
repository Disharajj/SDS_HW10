...
```{r echo = FALSE}
groceries_data$New_Store <- paste(groceries_data$Store, groceries_data$Neighborhood, sep = " ")
```

```{r echo = FALSE}
product_store_counts <-groceries_data %>%
  group_by(Product) %>%
  summarise(num_stores = n_distinct(New_Store))
```
...
```
  ...
  - Since the Income10K coefficient is negativ, the consumers in poorer zip codes tend to pay more for the same product.
